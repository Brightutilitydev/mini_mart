...
import { ShoppingCart, Minus, Plus, Trash2, CheckCircle2, AlertCircle, ShoppingBag, ArrowLeft, MapPin, CreditCard, Phone, Navigation } from 'lucide-react';
...
export default function Cart({ cart, clearCart, updateQuantity, removeFromCart, user }) {
  ...
  // Use browser Geolocation API to auto-generate a Google Maps link for dispatch
  const handleGetLocation = () => {
    ...
    if ('geolocation' in navigator) {
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          alert('Could not get your exact location. Please ensure location permissions are allowed.');
          setIsLocating(false);
        },
  ...
  const handleCheckout = async () => {
    ...
    if (!deliveryAddress && !gpsLink) {
      setOrderStatus({ type: 'error', message: 'Please provide a delivery address or your GPS location.' });
      return;
    }

    if (!contactPhone) {
      setOrderStatus({ type: 'error', message: 'A contact phone number is required for delivery.' });
      return;
    }
    ...
    try {
      // Send the payload including the new delivery tracking information
      ...
      <main className="flex-grow max-w-5xl mx-auto px-4 py-8 w-full">
        ...
        {cart.length === 0 ? (
          <div className="bg-white rounded-xl border border-gray-100 p-12 text-center shadow-sm">
            ...
            <p className="text-gray-400 text-sm mb-6">Browse our categories and add products to get started.</p>
            <Link 
              to="/"
              className="inline-block bg-[#f68b1e] text-white px-8 py-3 rounded-lg font-bold hover:bg-orange-600 transition-colors shadow-sm"
            >
              START SHOPPING
            </Link>
          </div>
            ...
            <div className="flex-grow space-y-4">
              {cart.map(item => (
                <div key={item.id} className="bg-white rounded-xl border border-gray-100 p-4 flex items-center gap-4 shadow-sm">
                  <div className="w-20 h-20 bg-gray-50 rounded-lg flex items-center justify-center border border-gray-100 flex-shrink-0">
                    {item.image_url ? (
                  ...
                  <div className="flex-grow">
                    ...
                    <p className="text-xs text-gray-400 mt-0.5">{item.brand || 'Generic Brand'}</p>
                  ...
                  <div className="flex flex-col sm:flex-row items-center gap-3">
                    <div className="flex items-center gap-2 border border-gray-200 rounded-lg bg-gray-50 p-1">
                      <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="p-1.5 hover:text-[#f68b1e] transition-colors">
                    ...
                    <button onClick={() => removeFromCart(item.id)} className="p-2 text-gray-400 hover:text-red-500 transition-colors bg-white rounded-lg border border-transparent hover:border-red-100 hover:bg-red-50" title="Remove Item">
            ...
            <div className="w-full lg:w-[22rem] h-fit sticky top-24 space-y-4">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <textarea 
                      ...
                      className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-[#f68b1e] outline-none transition-all resize-none h-16"
                    />
                  </div>

                  <div>
                    <label className="block text-xs font-bold text-gray-500 uppercase mb-2 flex items-center gap-1">
                      <Navigation className="h-3 w-3" /> Dispatch Map Link
                    </label>
                    {gpsLink ? (
                      <div className="flex items-center justify-between bg-green-50 border border-green-200 p-3 rounded-lg">
                        ...
                        <button onClick={() => setGpsLink('')} className="text-xs text-red-500 hover:underline">Remove</button>
                      </div>
                    ) : (
                      <button 
                        onClick={handleGetLocation}
                        disabled={isLocating}
                        className="w-full py-2.5 px-4 bg-gray-50 border border-gray-200 text-gray-700 rounded-lg text-sm font-bold hover:bg-gray-100 transition-colors flex items-center justify-center gap-2"
                      >
              ...
              <div className="bg-white rounded-xl border border-gray-100 p-6 shadow-sm">
                ...
                <div className="flex justify-between text-gray-500 text-sm mb-4">
                  ...
                  <span className="text-green-600 text-xs font-semibold bg-green-50 px-2 py-0.5 rounded-full">FREE</span>
                </div>
                ...
                <div className="flex justify-between text-gray-900 font-extrabold text-lg border-t border-gray-100 pt-4 mb-6">
                  ...
                  <span className="text-[#f68b1e]">₦{total.toLocaleString()}</span>
                </div>
                ...
                {user ? (
                  <button 
                    ...
                    disabled={isOrdering}
                    className="w-full bg-[#f68b1e] text-white py-3.5 rounded-lg font-black hover:bg-orange-600 transition-all shadow-md transform hover:scale-[1.01] flex items-center justify-center text-lg"
                  >
                    {isOrdering ? 'PLACING ORDER...' : 'PLACE ORDER NOW'}
                  </button>
                ) : (
                  <div className="space-y-3">
                    <p className="text-xs text-red-500 text-center font-medium">Please log in to finalize your order.</p>
                    <Link 
                      to="/auth"
                      className="w-full bg-gray-100 text-gray-700 py-3 rounded-lg font-bold hover:bg-gray-200 transition-colors text-sm text-center block"
                    >
                      LOG IN TO CHECKOUT
                    </Link>
                  </div>
