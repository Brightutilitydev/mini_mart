...
import { Await, Outlet, useRouteLoaderData, Navigate } from 'react-router'
...
export const Layout = memo(() => {
    ...
    return <>
     <Header />
          <main className="main" aria-hidden={modalIsActive} inert={modalIsActive}>
...
export const AuthLayout = () => {
  const { user_promise } = useRouteLoaderData('root')

  return <Suspense fallback={<LoadingScreen />}>
    <Await resolve={user_promise} errorElement={<Layout />}>{
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (e: any) => {return !e ? <Layout /> : <Navigate to={'/'}/>}
    }</Await>
  </Suspense>
}